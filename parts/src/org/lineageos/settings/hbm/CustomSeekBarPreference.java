...
public class CustomSeekBarPreference extends SliderPreference {
    ...
    private void initDefaults() {
        ...
        // setHapticFeedbackMode(HAPTIC_FEEDBACK_MODE_ON_TICKS); // Might not be available in all versions, check if needed
    ...
    private void attachResetIcon(TextView endText) {
        // If we have a default value, we might want to show a reset icon or text
        // The original code used a reset icon.
        // For now, we can leave this empty or implement if needed.
        // The patch didn't show the implementation of attachResetIcon fully in the snippet,
        // but it called it.
        // Wait, the snippet ended before attachResetIcon implementation.
        // I need to check if I have the implementation or if I should implement it.
        // The patch snippet ended at gcd.
        // I will implement a simple version or omit if not critical.
        // Actually, I should try to read the rest of the patch if possible.
        // But I can't.
        // However, looking at the code `if (endText != null) { attachResetIcon(endText); }`,
        // it seems it modifies the endText to show reset option.
        // I'll implement a basic version that sets an OnClickListener to reset to default.
        
        if (mDefaultValueExists) {
             endText.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ic_custom_seekbar_reset, 0);
    ...
    private void updatePlusMinusEnabledStates(PreferenceViewHolder holder) {
        ...
        if (minusIcon != null) {
            minusIcon.setEnabled(getValue() > getMin());
            minusIcon.setAlpha(minusIcon.isEnabled() ? 1.0f : 0.5f);
        }
        if (plusIcon != null) {
            plusIcon.setEnabled(getValue() < getMax());
            plusIcon.setAlpha(plusIcon.isEnabled() ? 1.0f : 0.5f);
        }
    }
}
